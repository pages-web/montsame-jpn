...
function OronNutagMainPost({ post, lang }: { post: any; lang: any }) {
...
function OronNutagSubPost({ post, lang }: { post: any; lang: any }) {
...
export default function OronNutag() {
  const { lang } = useLang();

  const { translated: headerTitle } = useTranslateText('Орон нутаг', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: { categoryIds: ["PKhKsHZLShXo8pf_8tot3"], sortField: "createdAt", sortDirection: "DESC", status: "published", limit: 3 },
  });
  ...
  return (
    <div>
      <Link href="/news?categoryId=PKhKsHZLShXo8pf_8tot3" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <OronNutagMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <OronNutagSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=PKhKsHZLShXo8pf_8tot3" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
