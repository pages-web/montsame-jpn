...
function EdiinZasagMainPost({ post, lang }: { post: any; lang: any }) {
...
function EdiinZasagSubPost({ post, lang }: { post: any; lang: any }) {
  ...
  return (
    <Link href={`/article/${post.slug || post._id}`} className="group flex gap-3 mb-2 border-b border-gray-300 pb-2">
      ...
      <div className="relative w-20 h-20 shrink-0 overflow-hidden">
        <Image src={IMG(post.thumbnail?.url)} alt={title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="80px" />
      </div>
    </Link>
  );
}

export default function EdiinZasag() {
  const { lang } = useLang();
  const { translated: headerTitle } = useTranslateText('Эдийн засаг', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);
  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: { categoryIds: ["gVqAdFTB-aHJoSuG-u16I"], sortField: "createdAt", sortDirection: "DESC", status: "published", limit: 3 },
  });
  ...
  return (
    <div>
      <Link href="/news?categoryId=gVqAdFTB-aHJoSuG-u16I" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <EdiinZasagMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <EdiinZasagSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=gVqAdFTB-aHJoSuG-u16I" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
