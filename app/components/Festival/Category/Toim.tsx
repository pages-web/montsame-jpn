...
function ToimMainPost({ post, lang }: { post: any; lang: any }) {
...
function ToimSubPost({ post, lang }: { post: any; lang: any }) {
...
export default function Toim() {
  const { lang } = useLang();

  const { translated: headerTitle } = useTranslateText('Тойм', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: { categoryIds: ["YbR8wobCtvEfajfBKzfTj"], sortField: "createdAt", sortDirection: "DESC", status: "published", limit: 3 },
  });
  ...
  return (
    <div>
      <Link href="/news?categoryId=YbR8wobCtvEfajfBKzfTj" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <ToimMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <ToimSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=YbR8wobCtvEfajfBKzfTj" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
