...
function AylalMainPost({ post, lang }: { post: any; lang: any }) {
...
function AylalSubPost({ post, lang }: { post: any; lang: any }) {
...
export default function Aylal() {
  const { lang } = useLang();
  const { translated: headerTitle } = useTranslateText('Аялал жуулчлал', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: { categoryIds: ["s9pxcf-65qz0EQYIunJWX"], sortField: "createdAt", sortDirection: "DESC", status: "published", limit: 3 },
  });
  ...
  return (
    <div>
      <Link href="/news?categoryId=s9pxcf-65qz0EQYIunJWX" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <AylalMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <AylalSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=s9pxcf-65qz0EQYIunJWX" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
