...
function DelkhiinMedeeMainPost({ post, lang }: { post: any; lang: any }) {
...
function DelkhiinMedeeSubPost({ post, lang }: { post: any; lang: any }) {
...
export default function DelkhiinMedee() {
  const { lang } = useLang();

  const { translated: headerTitle } = useTranslateText('Дэлхийн мэдээ', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: { categoryIds: ["ZQk0ST4J1PYmy2JAw_JqR"], sortField: "createdAt", sortDirection: "DESC", status: "published", limit: 3 },
  });
  ...
  return (
    <div>
      <Link href="/news?categoryId=ZQk0ST4J1PYmy2JAw_JqR" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <DelkhiinMedeeMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <DelkhiinMedeeSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=ZQk0ST4J1PYmy2JAw_JqR" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
