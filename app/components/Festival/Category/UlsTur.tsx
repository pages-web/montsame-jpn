...
function UlsTurMainPost({ post, lang }: { post: any; lang: any }) {
...
function UlsTurSubPost({ post, lang }: { post: any; lang: any }) {
...
export default function UlsTur() {
  const { lang } = useLang();

  // ← Hooks-ийг энд дээд хэсэгт дуудна, JSX дотор биш
  const { translated: headerTitle } = useTranslateText('Улс төр', lang);
  const { translated: moreText } = useTranslateText('дэлгэрэнгүй', lang);

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ["X3hU_0GHy3hgTft0LX0L2", "JwSGdqoU8jrsJb6jJ8qz1"],
      sortField: "createdAt",
      sortDirection: "DESC",
      status: "published",
      limit: 3,
  ...
  return (
    <div>
      <Link href="/news?categoryId=X3hU_0GHy3hgTft0LX0L2" className="text-[#0C4DA2] font-bold text-lg mb-4 hover:underline block">
        {headerTitle}
      </Link>

      <UlsTurMainPost post={posts[0]} lang={lang} />

      {posts.slice(1, 3).map((post: any) => (
        <UlsTurSubPost key={post._id} post={post} lang={lang} />
      ))}

      <Link href="/news?categoryId=X3hU_0GHy3hgTft0LX0L2" className="mt-3 text-sm text-gray-800 font-semibold hover:text-[#0C4DA2] transition-colors after:content-['_↗']">
        {moreText}
      </Link>
    </div>
  );
