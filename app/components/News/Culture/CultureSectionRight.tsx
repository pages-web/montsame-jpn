...
function SubPost({ post, lang }: { post: any; lang: any }) {
  ...
  return (
    <Link href={`/article/${post.slug || post._id}`} className="group block p-3 hover:bg-gray-50 transition-colors flex-1">
      <p
        className={`group-hover:text-[#0C4DA2] transition-colors ${isLoading ? 'opacity-50' : 'opacity-100'}`}
...
export default function CultureSectionRight() {
  const { lang } = useLang();

  const { data, loading, error } = useQuery(queries.cmsPostList, {
    variables: {
      categoryIds: ['mLrO7XWtdzsPE6mh8-Yxa'],
      sortField: 'createdAt',
      sortDirection: 'DESC',
      status: 'published',
      limit: 4,
